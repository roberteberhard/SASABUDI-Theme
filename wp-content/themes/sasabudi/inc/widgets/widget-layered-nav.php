 ...
 */
class SASA_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( '[Sasabudi] Display a list of attributes to filter products in your store.', 'woocommerce' );
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			/**
			 * Custom enhencements starts here
			 */
			$control_class = 'control-term';
			$control_label = '';
			$control_color = '';
			$control_theme = '';
			$control_slug  = '';
			...
			/**
			 * Attend the theme image & name to the themes controls.
			 */
			 if( $taxonomy == 'pa_theme') {
				$control_class = 'control-term__theme';
				$control_slug = esc_html($term->slug);
				$control_theme = ' ' . $control_slug;
				$control_thumb = get_field('shop_theme_icon', $term->taxonomy . '_' . $term->term_id);
				$control_image = '<span class="style-icon" style="background-image: url(' . $control_thumb . ')"></span>';
			}

			/**
			 * Attend the 'hexcolor' to the colors controls background.
			 */
			if( $taxonomy == 'pa_color') {
				$control_class = 'control-term__color';
				$control_label = $term->name;
				$variant_color = get_field('shop_product_color', $term->taxonomy . '_' . $term->term_id);
				$control_color = 'style="background-color:' . $variant_color . '"';
				
				/** 
				 * Exeption for colorized
				 */
				if($variant_color == '#ffffff') { // must be #ffffff
					$control_class = 'control-term__color colorized';
					$control_color = '';
				}
				if($variant_color == '#d8c049') { // must be #d8c049
					$control_class = 'control-term__color golden';
					$control_color = '';
				}
				if($variant_color == '#d6d6d6') { // must be #d6d6d6
					$control_class = 'control-term__color silver';
					$control_color = '';
				}
			}
			...
			// Update output
			echo '<li title="' . $control_label . '" class="' . $control_class . '' . $control_theme . ( $option_is_set ? ' chosen' : '' ) . '"' . $control_color . '>';
				if ($control_image) echo $control_image;
