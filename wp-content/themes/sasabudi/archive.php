<?php
/**
 * The template for displaying the archive blog page.
 *
 * @package WordPress
 * @subpackage SASABUDI
 * @since 1.0.0
      ...
      if ( have_posts() ) :
        ...
        echo '<header class="blog-hero">';
          echo '<figure class="blog-hero__figure">';
            ...
            echo '<div class="blog-hero__figure--image lazy-bg" style="background-image: url(data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==)" width="100%" heigth="100%" alt="" data-src="' . esc_url($page_header_src) . '">';
          ...
          while ( have_posts() ) : the_post();
            ...
            if ( $counter > 0 ) { // Show all except first one!
              ...
              echo '<article class="article' . $leftmargin . '' . $middlemargin . '' . $rightmargin . '">';
                ...
                // More
                echo '<p>';
                  echo '<a class="read_more primary-link" href="' . get_permalink() . '">' . esc_html__('Read More', 'sasabudi') . '</a> <span class="post-date">' . get_the_time('m/j/y') . '</span>';
                echo '</p>';
